...
function Z_Curvature_multiple(train_dir,db_name,n_samples,n_row,n_col,patch_dim,patch_overlap,scale,multiple)
...
% count imgs
fg_dir_categories   =   dir(strcat(train_dir,'/Data_*'));
J                   =   length(fg_dir_categories);
num                 =   zeros(J,1);
...
for j = 1:J
    img_dir =   [dir(strcat(train_dir,'/',fg_dir_categories(j).name,'/*.jpg'));
        dir(strcat(train_dir,'/',fg_dir_categories(j).name,'/*.bmp'));
        dir(strcat(train_dir,'/',fg_dir_categories(j).name,'/*.png'));];
    num(j)    =   n_samples;
...
t = 1;
for j = 1:J
    disp(['-> class ' fg_dir_categories(j).name])
    ...
    for i=1:n_samples
        ...
        img = imread(strcat(train_dir,'/',...
            fg_dir_categories(j).name,'/',img_dir(idx(i,j)).name));
