% Testing Session of the classification framework exploiting WARCO on the ETHZ (HOC) dataset.
%
% REFERENCES
...
% parametres
train_dir       = './database/HOC/train'; % training path
test_dir        = './database/HOC/test';% testing path
ds_name         = 'HOC';
J               = 4;% number of classes
K               = 8;% (K)cross-validation parameter
d               = 13; % number of features used to build a covariance matrix
patch_dim       = 24; % WARCO (single) patch size 
patch_overlap   = .5; % WARCO patch overlapping 
n_row           = 132; % (normalized) number of rows for the imgs in the db. 
n_col           = 62;; % (normalized) number of colums for the imgs in the db.
