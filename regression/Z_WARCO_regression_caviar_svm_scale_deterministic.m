function Z_WARCO_regression_caviar_svm_scale_deterministic(root,train_dir,test_dir,ds_name,K,patch_dim,patch_overlap,n_row,n_col,scale,d,cv_idx)
% Regressor training and testing procedure exploiting WARCO with the Frobenius distance for the CAVIAR data.
%
% USAGE
%  Z_WARCO_regression_caviar_svm_scale_deterministic(root,train_dir,test_dir,ds_name,K,patch_dim,patch_overlap,n_row,n_col,scale,d,cv_idx)
%
% INPUTS
%  root          - root path
...
if(exist([experiment '/model.mat'],'file') > 0)
    ...
    label_data        = zeros(n_train,1); % input PAN orientations
    ...
    for t = 1:n_wins
        ...
        [train_kernel{t},F{t},kernel_reg{t},~] = ...
            Y_ESVR_Train_deterministic(ts_data, sin(deg2rad(label_data)),0,K,cv_idx);
