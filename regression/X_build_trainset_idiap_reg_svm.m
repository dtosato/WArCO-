function  X_build_trainset_idiap_reg_svm(data_dir,store_dir,or_label_train,patches,Id,d,FB,offset,scale,~)
% Build the regression training set according to the patch-based model WARCO for the IDIAP data
%
% USAGE
%  X_build_trainset_idiap_reg_svm(data_dir,store_dir,or_label_train,patches,Id,d,FB,offset,scale,~)
%
% INPUTS
...
% end
% X_build_trainset_idiap_reg_svm(train_dir,data_dir,patches,Id,d,FB,offset,scale,1);
%
% REFERENCES
...
for i   =   1:n_img % for all images 
    % load image
    img            =   imread([data_dir '/' or_label_train(i).name]);
