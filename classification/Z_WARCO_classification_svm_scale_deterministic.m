function Z_WARCO_classification_svm_scale_deterministic(train_dir,test_dir,ds_name,J,K,patch_dim,patch_overlap,n_row,n_col,scale,d,cv_idx)
% Classifier training and testing procedure exploiting WARCO with the Frobenius distance.
%
% USAGE
%  Z_ARCO_classification_svm_scale_deterministic(train_dir,test_dir,ds_name,J,K,patch_dim,patch_overlap,n_row,n_col,scale,d,cv_idx)
%
% INPUTS
...
common          = [common '_K' num2str(K) '_deterministic'];
...
n_scale = length(scale); % check for multiple resolution WARCO (typically is 1)
...
if(exist([experiment '/model.mat'],'file') > 0)
    ...
    for t = 1:n_wins%par
        ...
        [ F{t}, train_kernel{t},kernel_reg{t},w(t,:)] = ...
            Y_patch_learning_deterministic(training_set,class_num_train,K,J,t,d,cv_idx);
    end
    %matlabpool close
...
for j = 1:J % for each class
    ...
    %matlabpool(4)
    for t = 1:n_wins%par
        log_posterior_tmp{t} = Y_patch_testing(zeros(N,J),testing_set_class,kernel_reg{t},...
