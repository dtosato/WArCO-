function [train_out,F,R,accuracy,w] = Y_GSVM_Train_deterministic(train,y,c1_rate,K,J,class_num_train,cv_idx)
% Learn a multiclass kernel SVM in the space of SPD matrices using the closed form Riemanninan distance.
%
% USAGE
%  [train_out,F,R,accuracy,w] = Y_GSVM_Train_deterministic(train,y,c1_rate,K,J,class_num_train,cv_idx)
%
% INPUTS
    ...
    for i = 1:M
        ...
        for j = i:M
            if i ~= j
                B = train_tmp(:,:,j);
                %Riemannian distance
                D(i,j) = trace(logm(A^(-1/2)*B*A^(-1/2))^2);
                D(j,i) = D(i,j);
            end
        end
        tocStatus( ticId, i/M);
